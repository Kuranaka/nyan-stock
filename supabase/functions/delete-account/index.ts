...
async function deleteUserIcons(url: string, serviceRoleKey: string, userId: string): Promise<void> {
  ...
  for (const root of ['cats', 'products']) {
    const objects = await listStorageFolder(url, serviceRoleKey, `${root}/${userId}`);
    paths.push(...objects.filter((object) => !object.name.includes('.emptyFolderPlaceholder')).map((object) => `${root}/${userId}/${object.name}`));
  }
  if (paths.length === 0) return;
  const response = await fetch(`${url}/storage/v1/object/icons`, {
    method: 'DELETE',
...
async function listStorageFolder(url: string, serviceRoleKey: string, prefix: string): Promise<StorageObject[]> {
  const response = await fetch(`${url}/storage/v1/object/list/icons`, {
    method: 'POST',
    headers: { ...serviceHeaders(serviceRoleKey), 'Content-Type': 'application/json' },
