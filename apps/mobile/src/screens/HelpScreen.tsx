...
const sections = [
  {
    title: 'はじめに',
    items: [
      'ペットプロフィールを登録してから、フードやトイレ用品などの商品を追加します。',
      '在庫一覧ではペットを切り替えて表示できます。ペットの追加・編集は、一覧上部の「ペット」から行えます。',
    ],
  },
  {
    title: '商品と残り日数',
  ...
  },
  {
    title: '共有スペース',
    items: [
      '共有する人が設定から「共有スペースを作成」を押すと、共有スペースと共有コードが作成されます。共有コードを、家族など共有したい相手に渡してください。',
      '参加する側は共有コードと参加名を入力します。参加すると、この端末のペットプロフィール・在庫・購入履歴は共有データで上書きされます。確認後に参加してください。',
      ...
      'Plusの登録数上限解除は、購入した端末のプランに適用されます。共有スペース全体にPlusが適用されるわけではありません。',
      '無料プランの参加者も、既存の共有データの閲覧・編集・削除、補充や購入履歴の記録を利用できます。新規追加は、共有されているペットプロフィールが2件まで・在庫が10件までの範囲で行えます。',
    ],
  },
  {
    title: 'データとアカウント',
