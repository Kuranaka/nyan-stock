...
const sections = [
  {
    title: '1. 取得する情報',
    body: [
      '本アプリでは、利用者が入力したペットプロフィール、在庫情報、購入履歴、商品リンク、通知設定、EC API設定などを取り扱います。',
      ...
      'GoogleまたはAppleによるログイン、匿名ゲスト参加、共有コードによるクラウド同期を利用する場合、認証情報、共有スペースID、ペットプロフィール、在庫情報、購入履歴などが、同期機能を提供するためSupabase上に保存される場合があります。',
      ...
      '利用者が必要な同意をした場合、これらの情報を、他社のアプリやWebサイトをまたぐ広告のパーソナライズおよび広告効果の測定に利用することがあります。ペットプロフィール、在庫情報、購入履歴、商品リンク、Googleログイン情報その他利用者が本アプリに入力した情報を、運営者が広告のターゲティング目的でGoogle AdMobへ送信することはありません。',
...
export default function PrivacyPolicyScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <AppCard>
        <Text style={styles.meta}>施行日: 2026年7月23日</Text>
        <Text style={styles.title}>プライバシーポリシー</Text>
        <Text style={[styles.text, { width: textWidth }]}>
          にゃんストック運営（以下「運営者」といいます。）は、ペット用品の在庫管理・買い忘れ防止アプリ「にゃんストック」（以下「本アプリ」といいます。）における利用者情報の取扱いについて、以下のとおり定めます。
        </Text>
